...
template <typename T, typename Tag>
struct AddEndianEnum;

// KeepTag specializations

template <>
struct AddEndian<u8, KeepTag> {
    using type = u8;
};

template <>
struct AddEndian<u16, KeepTag> {
    using type = u16;
};

template <>
struct AddEndian<u32, KeepTag> {
    using type = u32;
};

template <>
struct AddEndian<u64, KeepTag> {
    using type = u64;
};

template <>
struct AddEndian<s8, KeepTag> {
    using type = s8;
};

template <>
struct AddEndian<s16, KeepTag> {
    using type = s16;
};

template <>
struct AddEndian<s32, KeepTag> {
    using type = s32;
};

template <>
struct AddEndian<s64, KeepTag> {
    using type = s64;
};

template <>
struct AddEndian<float, KeepTag> {
    using type = float;
};

template <>
struct AddEndian<double, KeepTag> {
    using type = double;
};

template <typename T>
struct AddEndianEnum<T, KeepTag> {
    using type = T;
};
...
template <typename T>
struct AddEndianEnum<T, SwapTag> {
...
template <typename T>
using enum_le = std::enable_if_t<std::is_enum_v<T>, typename AddEndianEnum<T, LETag>::type>;
...
template <typename T>
using enum_be = std::enable_if_t<std::is_enum_v<T>, typename AddEndianEnum<T, BETag>::type>;
