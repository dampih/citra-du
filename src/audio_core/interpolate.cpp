...
void StepOverSamples(State& state, StereoBuffer16& input, float rate,
    ...
    input.insert(input.begin(), state.xni.begin(), state.xni.end());
    ...
    state.xni[0] = input[inputi];
    state.xni[1] = input[inputi + 1];
...
void Linear(State& state, StereoBuffer16& input, float rate, DSP::HLE::StereoFrame16& output,
    ...
    return StepOverSamples(state, input, rate, output, outputi,
