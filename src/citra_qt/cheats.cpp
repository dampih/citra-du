...
CheatDialog::CheatDialog(QWidget* parent)
    ...
    connect(ui->textDetails, &QPlainTextEdit::textChanged, this, &CheatDialog::OnTextEdited);
...
bool CheatDialog::SaveCheat(int row) {
    ...
    auto cheat = std::make_shared<Cheats::GatewayCheat>(
        ui->lineName->text().toStdString(), ui->textDetails->toPlainText().toStdString(),
...
void CheatDialog::OnRowSelected(int row, int column) {
    ...
    if (row < cheats.size()) {
        ...
        ui->textDetails->setPlainText(QString::fromStdString(current_cheat->GetCode()));
    }
